...
import com.avister.player.drawerItems.ExpandableSwitchDrawerItem
import com.avister.player.sheets.ClefSymbol
...
class SheetMusicActivity : MidiHandlingActivity() {
    lateinit var player /* The play/stop/rewind toolbar */: MidiPlayer
    lateinit var piano /* The piano at the top */: Piano
    lateinit var sheet /* The sheet music */: SheetMusic
    lateinit var layout /* The layout */: LinearLayout
    lateinit var midifile /* The midi file to play */: MidiFile
    lateinit var options /* The options for sheet music and sound */: MidiOptions
    var midiCRC /* CRC of the midi bytes */: Long? = null
...
//        sheet?.let{layout.removeView(it)}
//        layout.removeView(sheet)
    ...
    fun drawerItemClickListener(item: IDrawerItem<*>): Boolean {
        when (item.identifier.toInt()) {
            ...
            ID_LOOP_START -> {
                ...
                var items = makeStringList(1, options!!.lastMeasure + 1)
                builder.setItems(items) { dialog: DialogInterface?, i: Int ->
                    options!!.playMeasuresInLoopStart = items[i]!!
                        .toInt() - 1
                    ...
                    if (options!!.playMeasuresInLoopStart > options!!.playMeasuresInLoopEnd) {
                        ...
                        drawer!!.updateBadge(
                            ID_LOOP_END.toLong(), StringHolder(
                                items[i]
                            )
                        )
                    }
    ...
    private fun changeSettings() {
        val defaultOptions = MidiOptions(midifile)
    ...
    private fun showSaveImagesDialog() {
        ...
        builder.setPositiveButton(
            "OK"
        ) { builder1: DialogInterface?, _: Int -> saveAsImages(filenameView.text.toString()) }
        builder.setNegativeButton(
            "Cancel"
        ) { builder12: DialogInterface?, _: Int -> }
    ...
    private fun saveAsImages(name: String) {
        ...
        try {
            ...
            Toast.makeText(this, "Error: unsupported encoding in filename", Toast.LENGTH_SHORT)
                .show()
        }
        ...
        try {
            ...
            for (page in 1..numpages) {
                ...
                val path =
                    Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES + "/avister.player")
            ...
            val builder = AlertDialog.Builder(this)
            builder.setMessage("Error saving image to file " + Environment.DIRECTORY_PICTURES + "/avister.player/" + filename + ".png")
            ...
            val builder = AlertDialog.Builder(this)
            builder.setMessage("Ran out of memory while saving image to file " + Environment.DIRECTORY_PICTURES + "/avister.player/" + filename + ".png")
            builder.setCancelable(false)
            builder.setPositiveButton("OK") { dialog: DialogInterface?, id: Int -> }
            val alert = builder.create()
            alert.show()
